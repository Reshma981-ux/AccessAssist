...
export default function AccessAssistApp({ onSignOut }) {
  ...
  const handleReportBarrier = () => {
    if (!selectedPlace) return;
    const updated = { ...selectedPlace, barrier: { reportedAt: virtualNow, expiresAt: virtualNow + BARRIER_DURATION_MS, confirmations: 0, fixed: 0 }, verifiedAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  const handleResolveBarrier = () => {
    if (!selectedPlace) return;
    const updated = { ...selectedPlace, barrier: null, verified: true, verifiedAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    setPlaces((prev) => prev.map((p) => p.id === updated.id ? updated : p));
    persistPlace(updated);
  ...
  const confirmBarrier = (stillPresent) => {
    ...
    const updated = { ...selectedPlace, barrier, verifiedAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    setPlaces((prev) => prev.map((p) => p.id === updated.id ? updated : p));
    persistPlace(updated);
